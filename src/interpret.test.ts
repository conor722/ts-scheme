...
test("evaluates a basic expression", () => {
  expect(interpret(parse("(begin (define r 10) (* pi (* r r)))"))).closeTo(
    314.159,
    0.001
  );
});
...
test("evaluates a multiple conditional", () => {
  expect(interpret(parse("(begin (cond((= 1 2) 3) ((= 4 4) 2)))"))).toEqual(2);
});

test("evaluates a multiple conditional with an else clause", () => {
  expect(
    interpret(parse("(begin (cond((= 1 2) 3) ((= 4 3) 2) (else 9)))"))
  ).toEqual(9);
});
...
test("runs a function defined in code", () => {
  expect(
    interpret(parse("(begin (define (add-1 x) (+ 1 x)) (add-1 4))"))
  ).toEqual(5);
});
...
test("handles rest arguments", () => {
  expect(
    interpret(
      parse(
        "(begin (define (contrived-list . args) (apply list args)) (contrived-list 1 2 3 4))"
      )
    )
  ).toEqual([1, 2, 3, 4]);
});
...
test("lambda as an argument", () => {
  expect(
    interpret(parse("(apply (lambda (x y z) (+ x y z)) (list 1 2 3))"))
  ).toEqual(6);
});

test("lambda as a returned value", () => {
  expect(
    interpret(parse("(define (add-x x) (lambda (y) (+ x y))) ((add-x 4) 5)"))
  ).toEqual(9);
});
...
test("map", () => {
  expect(
    interpret(
      parse(
        "(define (map fn args)(cond ((empty? args) (list)) (else (push-left (map fn (cdr args)) (fn (car args))))))" +
...
test("error message includes line number for undefined variable", () => {
  const code = `(define x 10)
(define y 20)
(+ x y z)`;

  expect(() => interpret(parse(code))).toThrowError(
    /Undefined variable: z[\s\S]*at line 3/
  );
});

test("error message includes line number for unexpected closing paren", () => {
  const code = `(define x 10))
(+ x 5)`;

  expect(() => parse(code)).toThrowError(/Unexpected \)[\s\S]*at line 1/);
});

test("error message includes line number for function argument mismatch", () => {
  const code = `(define (add-two a b)
  (+ a b))

(add-two 1 2 3)`;

  expect(() => interpret(parse(code))).toThrowError(
    /function add-two takes 2 arguments, received 3/
  );
});
